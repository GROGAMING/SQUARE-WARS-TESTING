...
export function chooseComputerMove({ grid, blockedCells, aiDifficulty }) {
  ...
  if (aiDifficulty === "impossible") {
    return getBestMoveImpossible(grid, blockedCells);
  }
  return getBestMoveMedium(grid, blockedCells);
}
...
/** Center-first move ordering; limit caps branching for speed */
function getCandidateMovesOrdered(grid, blockedCells, limit = 12) {
  const candidates = [];
...
/** Adaptive depth: shallower early, deeper late (keeps UI responsive) */
function effectiveDepth(baseDepth, grid, blockedCells, hardMax = baseDepth) {
  let filled = 0;
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++)
      if (grid[r][c] !== 0 && !blockedCells.has(`${r}-${c}`)) filled++;
  }
...
/* Simpler eval for medium/beginner (unchanged enough) */
...
function minimaxAdvanced(
  ...
  const candidateMoves = getCandidateMovesOrdered(board, blockedCells, 8); // tighter for speed
...
function minimax(board, blockedCells, depth, isMaximizing, alpha, beta) {
  ...
  const candidateMoves = getCandidateMovesOrdered(board, blockedCells, 10);
...
function getBestMoveMedium(grid, blockedCells) {
  ...
  const moves = [];
  const candidates = getCandidateMovesOrdered(grid, blockedCells, 10);
...
function getBestMoveAdvanced(grid, blockedCells) {
  ...
  const depth = effectiveDepth(base, grid, blockedCells, base); // 4→3 early, 4 mid, 4 late
  ...
  const candidates = getCandidateMovesOrdered(grid, blockedCells, 8);
  const moves = [];
...
function getBestMoveImpossible(grid, blockedCells) {
  const base = AI.IMPOSSIBLE_DEPTH; // 6 by default
  const depth = effectiveDepth(base, grid, blockedCells, base); // 5 early, 6 mid/late
