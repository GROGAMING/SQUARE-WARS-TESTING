// ===== SQUARE WARS — ui.js (single SVG overlay for boxes) =====
import {
  UI_IDS,
  CSS,
  CELL,
...
export function buildGrid(rows, cols, onColumnClick) {
  ...
  // Ensure the overlay SVG exists (one-time)
  ensureBoxesSvg();
}

/**
 * Update a single cell with a ghost chip animation.
 * Only the latest move token may apply .last-move.
 */
export function updateCellDisplay(
  ...
  // Drop any existing last-move rings immediately
  ...
  // keep the real cell white while the ghost falls
  cell.className = "cell";
  cell.dataset.ghost = "1";

  // Ghost chip falls inside outlineLayer (absolute)
  ...
  const finish = () => {
    ...
    if (isBlocked) {
      ...
      if (token === uiLastMoveToken) {
        // only newest move gets the ring
        document
...
/** Update every cell (used after blocking areas) */
export function updateAllCellDisplays(
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      // Keep white during a ghost-drop
  ...
  // Re-apply last move marker if applicable
  if (
    lastMovePosition &&
...
function ensureBoxesSvg() {
  ...
  // red: base 255,107,107 ; blue: 77,171,247
...
/**
 * Draw/append a rectangle into the single SVG overlay.
 */
export function drawOutlineRect(minRow, maxRow, minCol, maxCol, player) {
  ...
  // Grid → pixel mapping (same math as before)
  ...
  rect.setAttribute("fill", `url(#${isRed ? "hatch-red" : "hatch-blue"})`);
  rect.setAttribute("stroke", isRed ? "#ff6b6b" : "#4dabf7");
  rect.setAttribute("stroke-width", "3");
  rect.setAttribute("filter", `url(#${isRed ? "glow-red" : "glow-blue"})`);
  rect.setAttribute("opacity", "1");

  svg.querySelector("#boxesGroup").appendChild(rect);
}

/** Sleek persistent strike through the winning line */
